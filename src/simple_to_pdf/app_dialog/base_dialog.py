...
class BaseDialog(tk.Toplevel,ThemeProviderMixin):
    ...
    def _init_layout(self):
        ...
        self.content = BaseFrame(self, frame_type="content", padx=35, pady=20)
        self.content.pack(fill="both", expand=True)
        
        # Footer: Bottom area for secondary labels or small buttons
        self.footer = BaseFrame(self, frame_type="footer", pady=15)
    ...
    def set_header_text(self, title: str, subtitle: str|None = None):
        ...
        BaseLabel(
            self.header, text=title, label_type='title'
        ).pack(pady=(25, 2))
        
        if subtitle:
            BaseLabel(
                self.header, text=subtitle,label_type='badge'
            ).pack(pady=(0, 20))
