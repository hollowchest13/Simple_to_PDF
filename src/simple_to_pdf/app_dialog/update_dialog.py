...
class UpdateDialog(BaseDialog):
    ...
    def __init__(self, parent, new_version, changelog):
        ...
        # Content area
        BaseLabel(self.content, text="What's New:").pack(anchor="w", pady=(0, 5))

        # Text area for changelog (could be a simple label or ScrolledText)
        change_log_box = BaseLabel(self.content, text=changelog, label_type="badge")
        ...
        PrimaryButton(
            btn_frame, text="Download Now", command=self._on_update_click
        ).pack(side="right", padx=5)

        PrimaryButton(btn_frame, text="Later", command=self.destroy).pack(
            side="right", padx=5
        )
